...
// @ts-expect-error SIP transport is not a root export.
import type { SipTransport } from "ringcentral-softphone";
...
void (undefined as unknown as SipTransport);
...
// @ts-expect-error SIP signaling state is not a supported declaration.
softphone.signaling;
// @ts-expect-error The old raw TLS socket is not a supported declaration.
softphone.client;
// @ts-expect-error Internal SIP transmission is not a supported declaration.
softphone.send;
