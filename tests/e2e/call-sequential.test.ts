...
describe("E2E sequential calls", () => {
  test("one registration completes two sequential calls with fresh Call-IDs and recognizable per-call media", async () => {
    ...
    let callerCallIdDistinctAcrossCalls: string = "not evaluated yet";
    let calleeCallIdDistinctAcrossCalls: string = "not evaluated yet";
    ...
    const describeFailure = (phase: string) =>
      [
        ...
        `Call-ID relationships (identifiers withheld): non-empty: ${callIdNonEmpty}, A distinct across calls: ${callerCallIdDistinctAcrossCalls}, B distinct across calls: ${calleeCallIdDistinctAcrossCalls}`,
    ...
    try {
      ...
      let previousCallerCallId: string | undefined;
      let previousCalleeCallId: string | undefined;
      ...
      for (const [index, toneHz] of [
        ...
        callerCallIdDistinctAcrossCalls =
          previousCallerCallId === undefined
            ? "not applicable before the first call"
            : "not evaluated yet";
        calleeCallIdDistinctAcrossCalls =
          previousCalleeCallId === undefined
            ? "not applicable before the first call"
            : "not evaluated yet";
        ...
        await boundedPhase(
          ...
          async () => {
            // A (caller) and B (callee) each own a SIP dialog on RingCentral's
            // B2BUA, so within one call the two legs' Call-IDs are never
            // compared to each other; freshness is checked per endpoint.
            const callerCallId = outbound.callId;
            const calleeCallId = inbound.callId;
            callIdNonEmpty = callerCallId !== "" && calleeCallId !== "";
            expect(
              callIdNonEmpty,
              "A and B Call-IDs must be non-empty (values withheld)",
            ).toBe(true);
            if (previousCallerCallId !== undefined) {
              callerCallIdDistinctAcrossCalls = String(
                callerCallId !== previousCallerCallId,
              );
              expect(
                callerCallIdDistinctAcrossCalls,
                "A's Call-ID must differ between calls one and two (values withheld)",
              ).toBe("true");
            }
            if (previousCalleeCallId !== undefined) {
              calleeCallIdDistinctAcrossCalls = String(
                calleeCallId !== previousCalleeCallId,
              );
              expect(
                calleeCallIdDistinctAcrossCalls,
                "B's Call-ID must differ between calls one and two (values withheld)",
              ).toBe("true");
            }
          },
        );
        ...
        previousCallerCallId = outbound.callId;
        previousCalleeCallId = inbound.callId;
      }
    } finally {
